...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if((r = openfile_lookup(envid, req->req_fileid, &po)) < 0)
		return r;
	if((r = file_read(po->o_file, ret->ret_buf, MIN(req->req_n, PGSIZE), po->o_fd->fd_offset)) < 0)
		return r;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	// LAB 5: Your code here.
	int r;
	size_t maxlen = PGSIZE - (sizeof(int) + sizeof(size_t));
	struct OpenFile *po;

	if((r = openfile_lookup(envid, req->req_fileid, &po)) < 0)
		return r;
	if((r = file_write(po->o_file, req->req_buf, MIN(req->req_n, maxlen), po->o_fd->fd_offset)) < 0)
		return r;
